...
public class Galli : Creature
{
	...
	void OnCollisionStay(Collision col)
	{
		...
		switch (rndPainsnd)
		{
			case 0: painSnd = Galli1; break;
			case 1: painSnd = Galli2; break;
			case 2: painSnd = Galli3; break;
			case 3: painSnd = Galli4; break;
		}
		...
		if (time == currframe && lastframe != currframe)
		{
			switch (name)
			{
				case "Step":
					...
					break;
				case "Bite":
					source[1].pitch = Random.Range(1.0f, 1.25f);
					source[1].PlayOneShot(Bite, 0.75f);
					lastframe = currframe;
					break;
				case "Die":
					source[1].pitch = Random.Range(0.8f, 1.0f);
					source[1].PlayOneShot(isOnWater | isInWater ? Smallsplash : Smallstep, 1.0f);
					lastframe = currframe;
					isDead = true;
					break;
				case "Food":
					source[0].pitch = Random.Range(3.0f, 3.5f);
				...
				case "GrowlA":
					...
					source[0].pitch = Random.Range(1.0f, 1.25f);
	...
	void FixedUpdate()
	{
		...
		//Jump
		else if (OnAnm.IsName("Galli|IdleJumpStart") | OnAnm.IsName("Galli|RunJumpStart") |
		         OnAnm.IsName("Galli|JumpIdle") |
		         OnAnm.IsName("Galli|IdleJumpEnd") | OnAnm.IsName("Galli|RunJumpEnd"))
		{
			...
			{
				if (OnAnm.normalizedTime > 0.4) Move(Vector3.up, 3, true);
				else onJump = true;
			...
			else if (OnAnm.IsName("Galli|IdleJumpEnd") | OnAnm.IsName("Galli|RunJumpEnd"))
			{
				...
				body.linearVelocity = new Vector3(body.linearVelocity.x, 0.0f, body.linearVelocity.z);
				onJump = false;
				PlaySound("Step", 3);
				PlaySound("Step", 4);
			}
