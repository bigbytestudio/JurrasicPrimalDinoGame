...
public class Came : Creature
{
  ...
  public AudioClip Waterflush,Hit_jaw,Hit_head,Hit_tail,Bigstep,Largesplash,Ammo1,Ammo2,Ammo3;
    ...
    //Set Y position
		if(isInWater)
		{
      body.linearDamping=1; body.angularDamping=1;
      ...
      anm.SetBool("OnGround", false);
		...
		else if(isOnGround) { body.linearDamping=4; body.angularDamping=4; anm.SetBool("OnGround", true); ApplyYPos(); }
    else
    {
      if(health!=0) { Move(Vector3.zero); pitch=Mathf.Lerp(pitch, anm.GetFloat("Pitch")*90f, ang_T); }
      anm.SetBool("OnGround", false); onJump=true; body.linearDamping=0.5f; body.angularDamping=0.5f; ApplyGravity();
    }
